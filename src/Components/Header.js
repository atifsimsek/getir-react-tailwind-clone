...
const Header = () => {
  ...
  return (
    <>
      ...
      <div className='sticky top-0 z-30 bg-brand-color flex items-center justify-center '>
        <div className=' container mx-auto h-16 md:h-11  flex items-center justify-center md:justify-between'>
          ...
          <nav className=' hidden md:flex gap-x-6 lg:gap-x-8 -mr-10 lg:-mr-0 '>
            <button onClick={() => { openModal("language") }} className=' flex gap-x-2 text-white transition-all text-sm font-semibold opacity-80 hover:opacity-100 '>
              <BiGlobe size={20} />
              <span className="hidden lg:inline">{language === "tr-TR" || language === "tr" ? "Türkçe" : "English"}</span>
            </button>
            <button onClick={() => { openModal("login") }} className='cursor-pointer flex gap-x-2 text-white transition-all text-sm font-semibold opacity-80 hover:opacity-100 '>
              <HiUser size={20} />
              <span className="hidden lg:inline">{language === "tr-TR" || language === "tr" ? "Giriş Yap" : "Login"}</span>
            </button>
            <button onClick={() => { openModal("register") }} className=' flex gap-x-2 text-white transition-all text-sm font-semibold opacity-80 hover:opacity-100 '>
              <HiUserAdd size={20} />
              <span className="hidden lg:inline"> {language === "tr-TR" || language === "tr" ? "Kayıt Ol" : "Register"}</span>
            </button>
          </nav>

        </div>
      </div>
      <div className='md:hidden bg-white mx-auto px-1.5 py-2.5 flex items-center justify-between'>
        <div className='flex items-center gap-1'>
          <TiLocation className='text-primary-brand-color' size={18} />
          <span className='text-sm text-gray-600 font-semibold'>Teslimat Adresi Belirle</span>
        </div>
        <MdArrowForwardIos className='text-primary-brand-color mr-0.5 ' size={16} />
      </div>
    </>
  )
}
