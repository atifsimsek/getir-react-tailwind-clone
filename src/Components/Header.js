/* eslint-disable jsx-a11y/alt-text */
import React from 'react'
import { BiGlobe } from "react-icons/bi"
import { TiLocation } from "react-icons/ti"
import { HiUser, HiUserAdd } from "react-icons/hi"
import { MdArrowForwardIos } from "react-icons/md"
import { createModal, useModals } from "../utils/modalUtils"
import Modal from "../modals"
const Header = () => {

  const modals = useModals()

  console.log(modals)

  const openModal = (name) => {
    createModal(name)
    document.body.style.overflow ="hidden"
  }

  return (
    <>
      {modals.length > 0 && <Modal />}
      <div className='sticky top-0 z-40 bg-brand-color flex items-center justify-center '>
        <div className=' container mx-auto h-16 md:h11  flex items-center justify-center md:justify-between'>
          <a href='/'>
            <img src="data:image/svg+xml;base64," />
          </a>
          <nav className=' hidden md:flex gap-x-8 '>
            <button onClick={()=>{openModal("language")}} className=' flex gap-x-2 text-white transition-all text-sm font-semibold opacity-80 hover:opacity-100 ' href="/">
              <BiGlobe size={20} />
              Türkçe
            </button>
            <buttton onClick={()=>{openModal("login")}} className=' flex gap-x-2 text-white transition-all text-sm font-semibold opacity-80 hover:opacity-100 ' href="/">
              <HiUser size={20} />
              Giriş Yap
            </buttton>
            <button onClick={()=>{openModal("register")}}  className=' flex gap-x-2 text-white transition-all text-sm font-semibold opacity-80 hover:opacity-100 ' href="/">
              <HiUserAdd size={20} />
              Kayıt Ol
            </button>
          </nav>

        </div>
      </div>
      <div className='md:hidden bg-white mx-auto px-1.5 py-2.5 flex items-center justify-between'>
        <div className='flex items-center gap-1'>
          <TiLocation className='text-primary-brand-color' size={18} />
          <span className='text-sm text-gray-600 font-semibold'>Teslimat Adresi Belirle</span>
        </div>
        <MdArrowForwardIos className='text-primary-brand-color mr-0.5 ' size={16} />
      </div>
    </>
  )
}

export default Header